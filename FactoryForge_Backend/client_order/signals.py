...
@receiver(pre_save, sender=ClientOrder)
def custom_user_pre_save(sender, instance, **kwargs):
    ...
    if instance.pk:
        ...
        if changes:
            print(f"Changes in Client Order instance with id {instance.id} during PATCH request:")
...
def custom_user_post_save(sender, instance, created, **kwargs):
    ...
    if created:
        print(f"New Client Order instance created.")
...
def get_patch_changes(instance):
    ...
    try:
        db_instance = ClientOrder.objects.get(pk=instance.pk)
    except ClientOrder.DoesNotExist:

        return changes


    for field in ClientOrder._meta.fields:
