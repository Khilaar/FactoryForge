...
def custom_user_pre_save(sender, instance, **kwargs):
    ...
    if instance.pk:
        ...
        if changes:
            print(f"Changes in Product instance with id {instance.id} during PATCH request:")
...
def custom_user_post_save(sender, instance, created, **kwargs):
    ...
    if created:
        print(f"New Product instance created.")
...
def get_patch_changes(instance):
    ...
    try:
        db_instance = Product.objects.get(pk=instance.pk)
