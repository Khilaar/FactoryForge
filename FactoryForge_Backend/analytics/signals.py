...
@receiver(pre_save, sender=Analytics)
def custom_user_pre_save(sender, instance, **kwargs):
    ...
    if instance.pk:
        ...
        if changes:
            print(f"Changes in Analytics instance with id {instance.id} during PATCH request:")
...
def custom_user_post_save(sender, instance, created, **kwargs):
    ...
    if created:
        print(f"New Analytics instance created.")
